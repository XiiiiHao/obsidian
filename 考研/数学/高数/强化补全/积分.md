...
* $\displaystyle \int \sqrt{ x^{2}-a^{2} }= \frac{x \sqrt{ x^{2}-a^{2} } }{2}- \frac{a^{2}}{2}\ln (x+ \sqrt{ x^{2}-a^{2} })+C$
* 
## 定理
> [!theorem]+ 第一积分中值定理
...
* 上下限相同考虑用中值定理
* 带绝对值的周期函数积分，可考虑找一个不同分开积分的区间
* $\displaystyle \int^{\pi}_{0}xf(\sin x)dx= \frac{\pi}{2}\int^{\pi}_{0}f(\sin x)dx$
* 善用$\displaystyle dx=d(x-t)$
* 变量代换的目的：区间保持不变
* 凑分母的导数跟本身的线性组合
* 积分不等式
	* 变量代换
